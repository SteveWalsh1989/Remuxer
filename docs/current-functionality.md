...
- `Lossless MP4` remuxes compatible video, audio, and MP4-compatible subtitle streams with `-c copy`. It tags copied HEVC video as `hvc1` for Apple playback compatibility. It blocks incompatible video and audio because those would require re-encoding or transcoding.
