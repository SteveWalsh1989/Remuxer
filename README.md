...
Remuxer is a native macOS app for converting `.mkv` files into more compatible video files with FFmpeg.
...
- Can remove original source files after a successful conversion when that option is enabled.

## Current Limitations

- Remuxer currently accepts `.mkv` input files only.
...
- Source files are removed only after successful conversion and sidecar extraction. If the output would replace the source file, conversion is blocked.
